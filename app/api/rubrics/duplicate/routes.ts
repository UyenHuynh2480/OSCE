...
export default function RubricsListPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: cs } = await supabase.from("cohorts").select("id,year,level_id").eq("level_id", levelId).order("year");
      setCohorts(cs ?? []);
    })();
  }, [levelId]);
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: rs } = await supabase.from("exam_rounds_view").select("id,display_name,cohort_id").eq("cohort_id", cohortId).order("display_name");
      setRounds(rs ?? []);
    })();
  }, [cohortId]);
  ...
  const onCopyClient = async (sourceId: string) => {
    // Nhập ngữ cảnh đích nhanh (có thể thay bằng modal sau)
  ...
  return (
    <div className="px-4 py-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={goCreate} className="rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700">
          + Tạo Rubric mới
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3 bg-white border border-gray-200 rounded-lg p-4">
        <select className="w-full rounded-md border px-3 py-2 text-sm"
          value={levelId} onChange={(e) => { setLevelId(e.target.value); setPage(1); }}>
          <option value="">Level (tất cả)</option>
          {levels.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}
        </select>

        <select className="w-full rounded-md border px-3 py-2 text-sm"
          value={cohortId} onChange={(e) => { setCohortId(e.target.value); setPage(1); }} disabled={!levelId}>
          <option value="">Cohort (tất cả)</option>
          {cohorts.map((c) => <option key={c.id} value={c.id}>{c.year}</option>)}
        </select>

        <select className="w-full rounded-md border px-3 py-2 text-sm"
          value={roundId} onChange={(e) => { setRoundId(e.target.value); setPage(1); }} disabled={!cohortId}>
          <option value="">Round (tất cả)</option>
          {rounds.map((r) => <option key={r.id} value={r.id}>{r.display_name}</option>)}
        </select>

        <select className="w-full rounded-md border px-3 py-2 text-sm"
          value={stationId} onChange={(e) => { setStationId(e.target.value); setPage(1); }}>
          <option value="">Station (tất cả)</option>
          {stations.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
        </select>

        <input type="text" placeholder="Tìm theo tên hiển thị..."
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => setPage((p) => Math.max(1, p - 1))}
                  className="rounded border px-2 py-1 text-sm hover:bg-gray-100" disabled={page <= 1}>← Trước</button>
          <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))}
                  className="rounded border px-2 py-1 text-sm hover:bg-gray-100" disabled={page >= totalPages}>Sau →</button>
        </div>
      </div>
    </div>
  );
